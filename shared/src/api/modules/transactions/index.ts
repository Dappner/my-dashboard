...
export const transactionsApiKeys = {
  ...
  ticker: (exchange: string, symbol: string) =>
    [...transactionsApiKeys.all, exchange, symbol] as const,
};

export function createTransactionsApi(supabase: SupabaseClient<Database>) {
  return {
    ...
    async deleteTransaction(id: string): Promise<void> {
      const { error } = await supabase.from("transactions").delete().eq(
        "id",
        id,
      );
