...
export function TickerForm({
  ...
  const onSubmit = (values: TickerFormValues) => {
    if (mode === "update") {
      updateTicker({ id: tickerId || undefined, ...values });
    } else {
      addTicker({ ...values, backfill: true } as InsertTicker);
    }
  };
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row gap-4">
                  ...
                  <FormControl>
                    {mode === "create" ? (
                      // For create mode: always checked and disabled
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          (Always runs for new tickers)
                        </span>
                      </div>
                    ) : (
                      // For update mode: normal checkbox behavior
                      <Checkbox
