...
export default function TransactionsFilters({ filters, setTransactionsFilters, onAddTransaction }: TransactionsFiltersProps) {
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  <Select
                    value={field.value?.from ? format(field.value.from, "MMM yyyy") : undefined}
                    onValueChange={(value) => {
                      const newFrom = monthOptions.find(opt => opt.label === value)?.value;
                      const newRange = {
                        from: newFrom,
                        to: field.value?.to
                      };
                      field.onChange(newRange.from || newRange.to ? newRange : null);
                      onFilterChange({ ...filters, dateRange: newRange.from || newRange.to ? newRange : null });
                    }}
                  >
                    <FormControl>
                      <SelectTrigger className={cn("w-[140px]", !field.value?.from && "text-muted-foreground")}>
                    ...
                    <SelectContent>
                      {monthOptions.map((option) => (
                        <SelectItem
                          key={option.label}
                          value={option.label}
                          disabled={field.value?.to && option.value > field.value.to}
                        >
                          {option.label}
                        </SelectItem>
                      ))}
                  ...
                  <span>-</span>
                  <Select
                    value={field.value?.to ? format(field.value.to, "MMM yyyy") : undefined}
                    onValueChange={(value) => {
                      const newTo = monthOptions.find(opt => opt.label === value)?.value;
                      const newRange = {
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className={cn("w-[140px]", !field.value?.to && "text-muted-foreground")}>
                    ...
                    <SelectContent>
                      {monthOptions.map((option) => (
                        <SelectItem
                          key={option.label}
                          value={option.label}
                          disabled={field.value?.from && option.value < field.value.from}
                        >
                          {option.label}
                        </SelectItem>
                      ))}
