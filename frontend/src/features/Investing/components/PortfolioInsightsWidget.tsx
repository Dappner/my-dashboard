...
export default function PortfolioInsightsWidget({
	...
	const bestPerformer = holdings?.reduce(
		(max, h) =>
			h.unrealized_gain_loss_percent! > max.value
				? { value: h.unrealized_gain_loss_percent!, symbol: h.symbol! }
				: max,
		{ value: Number.NEGATIVE_INFINITY, symbol: "" },
	);
	const worstPerformer = holdings?.reduce(
		(min, h) =>
			h.unrealized_gain_loss_percent! < min.value
				? { value: h.unrealized_gain_loss_percent!, symbol: h.symbol! }
				: min,
		{ value: Number.POSITIVE_INFINITY, symbol: "" },
	);

	const nextDividendEvent = events?.find(
		(e) => e.event_type === "dividend" && new Date(e.date) > new Date(),
	);
						...
						<div className="space-y-4">
							<div className="flex justify-between items-center">
								<span className="text-sm font-medium">
									Best Performer ({bestPerformer?.symbol})
								</span>
								<div className="flex items-center text-emerald-600">
									<span className="font-semibold">
										+{bestPerformer?.value.toFixed(1)}%
									</span>
							...
							<div className="flex justify-between items-center">
								<span className="text-sm font-medium">
									Worst Performer ({worstPerformer?.symbol})
								</span>
								<span className="text-red-600 font-semibold">
									{worstPerformer?.value.toFixed(1)}%
								</span>
							</div>
							<div className="flex justify-between items-center">
								<span className="text-sm font-medium">{timeframe} Return</span>
								<span
									className={`font-semibold ${metrics.periodTotalReturnPercent >= 0 ? "text-emerald-600" : "text-red-600"}`}
								>
									{metrics.periodTotalReturnPercent >= 0 ? "+" : ""}
									{metrics.periodTotalReturnPercent.toFixed(1)}%
								</span>
							</div>
						</div>
					</TabsContent>
					...
					<TabsContent value="dividends" className="pt-4">
						<div className="space-y-4">
							<div className="flex justify-between items-center">
								...
								<span className="font-semibold">
									${metrics.currentYearDividends?.toFixed(2) || "0.00"}
								</span>
							</div>
							<div className="flex justify-between items-center">
								<span className="text-sm font-medium">Dividend Yield</span>
								<span className="font-semibold">
									{metrics.currentDividendYield?.toFixed(1) || "0.0"}%
								</span>
							</div>
							<div className="flex justify-between items-center">
								...
								<div className="flex items-center">
									...
									<span className="font-semibold">
										{nextDividendEvent
											? format(new Date(nextDividendEvent.date), "MMM dd")
											: "N/A"}
									</span>
								</div>
							</div>
