...
export default function TotalValueDisplay({
  ...
  return (
    <div className={cn(className)}>
      <div className="hidden sm:block sm:text-xl font-normal text-muted-foreground">
        Value
      </div>
      <div className="text-3xl sm:text-4xl font-bold text-foreground mb-0.5">
        {formatCurrency(metrics?.currentTotalValue)}
      </div>

      <div
        className={`flex items-center text-sm space-x-1.5 ${isPositiveChange ? "text-green-600" : "text-red-600"
          }`}
      >
