...
const LoadingState = () => (
  <div className="flex items-center justify-center h-96 min-h-[384px] bg-background rounded-lg shadow-sm border">
...
export default function PortfolioChart({
      ...
      for (let i = 0; i < sortedMetrics.length; i++) {
        ...
        if (
          ...
          if (
            ...
            dailyBenchmarkReturnPct = (currentBenchmarkPrice / prevPrice - 1) *
              100;
  ...
  const yDomain = useMemo((): [number | string, number | string] => {
    ...
    const minValueInData = relevantValues.length > 0
      ? Math.min(...relevantValues)
      : 0;
  ...
  const formatLegendLabel = (value: string): React.ReactNode => {
    ...
    const shouldBeVisible = (type === "absolute" && key === "totalPortfolio") ||
  ...
  const isLoading = metricsLoading ||
    (shouldFetchHistorical && historicalPricesLoading);
        ...
        <ChartContainer config={chartConfig} className="h-full w-full">
          ...
          >
            ...
            <XAxis
              ...
              axisLine={false}
              tickMargin={8}
              tickFormatter={(val) => isValid(val) ? format(val, "MMM d") : ""}
              // Use CSS variables if defined, otherwise fallback
              stroke="hsl(var(--muted-foreground, 215.4 16.3% 46.9%))"
              fontSize={12}
              ...
              content={
                <ChartTooltipContent
                  ...
                  labelFormatter={(label, payload) => {
                    ...
                    const dateToFormat = dateFromPayload instanceof Date &&
