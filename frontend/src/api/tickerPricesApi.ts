...
export const tickerPricesApiKeys = {
  ...
  timeframe: (tickerId: string, timeframe: Timeframe) => [...tickerPricesApiKeys.ticker(tickerId), timeframe] as const,
};

export const tickerPricesApi = {
  async getTickerHistoricalPrices(tickerId: string, timeframe: Timeframe): Promise<HistoricalPrice[]> {
    const now = new Date();
    let query = supabase
